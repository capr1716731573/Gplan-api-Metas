...
//DATOS DE LA TABLA
var datos_tabla = {
    tabla_target: 'objetivo_pntv',
    pk_tabla: 'pk_obj',
    sp_crud_tabla: 'sp_gplan_crud_objetivos'
}
...
// ========================================== 
app.get('/:eje', mdAuthenticationJWT.verificarToken, (req, res, next) => {
    ...
    if (req.query.desde) {
        consulta = `SELECT * FROM ${ datos_tabla.tabla_target } WHERE fk_eje = ${eje} AND activo_obj=true ORDER BY numeral_obj ASC LIMIT ${ rows } OFFSET ${ desde }`;
    } else {
        consulta = `SELECT * FROM ${ datos_tabla.tabla_target } WHERE fk_eje = ${eje} AND activo_obj=true ORDER BY numeral_obj ASC`;
    }
...
// Obtener todos los registros TODOS x PADRE
// ========================================== 
app.get('/todos/:eje', mdAuthenticationJWT.verificarToken, (req, res, next) => {
    var desde = req.query.desde;
    var eje = req.params.eje;
    desde = Number(desde);
    var fk_padre = req.query.fk_padre || 0;
    ...
    //valido que exista el parametro "desde"
    if (req.query.desde) {
        consulta = `SELECT * FROM ${ datos_tabla.tabla_target } WHERE fk_eje = ${eje} ORDER BY numeral_obj ASC LIMIT ${ rows } OFFSET ${ desde }`;
    } else {
        consulta = `SELECT * FROM ${ datos_tabla.tabla_target } WHERE fk_eje = ${eje} ORDER BY numeral_obj ASC`;
    }
    crud.getAll(datos_tabla.tabla_target, consulta, res);
});
